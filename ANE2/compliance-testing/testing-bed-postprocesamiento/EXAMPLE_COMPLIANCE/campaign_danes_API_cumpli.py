...
def main():
    ...
    from modules.data_request import DataRequest
    from modules.danes_request import DaneTunnelClient
    ...
    dane_client = DaneTunnelClient()
    dane_codes = dane_client.get_dane_codes(lat, lon)
    if not dane_codes:
        raise RuntimeError(
            f"No fue posible resolver códigos DANE para lat={lat} lon={lon}."
        )
