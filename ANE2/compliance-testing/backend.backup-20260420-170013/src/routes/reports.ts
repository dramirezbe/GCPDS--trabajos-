...
router.post('/compliance/:campaignId', async (req: Request, res: Response) => {
  try {
    ...
    // Helper para procesar una sola medición de forma asíncrona
    const processMeasurement = async (frameData: any, i: number) => {
      let pxxArray: number[];
      try {
        ...
        console.error(`Error parsing Pxx for measurement ${i + 1}:`, parseError);
        return null;
      }
      ...
      try {
        const daneParam = codigo_dane || null;
        
        const pythonPayload = {
          frame: frame,
          cumplimiento: 1,
          dane: daneParam ? String(daneParam).trim() : null,
          danes: danesList,
          picos: [],
          umbral_db: UMBRAL_DB,
        ...
        const pythonResponse = await axios.post(`${PYTHON_SERVICE_URL}/analyze`, pythonPayload, {
          timeout: 3600000, // Aumentado a 1 hora (3600s)
          headers: { 'Content-Type': 'application/json' }
        });
        analysisResults = pythonResponse.data;
        
      } catch (pythonError: any) {
        console.error(`❌ Error analyzing measurement ${i + 1}:`, {
        ...
        return {
          measurement: {
            ...
            error: 'Error en análisis Python',
            error_detalle: pythonError.response?.data || pythonError.message,
            emisiones: []
          },
          stats: { total: 0, authorized: 0, unauthorized: 0 }
        };
      }
      ...
      if (analysisResults.results_by_dane && danesList.length > 1) {
        for (let k = 0; k < emissions.length; k++) {
          const original = emissions[k];
          if (original.Licencia === 'SI') continue;
      ...
      const emisionesFormateadas = emissions.map((emission: any) => {
        ...
        return {
          ...
          cumple_bw: cumpleBW,
          licencia: licencia,
          dane_asociado: emission._dane_autorizado || codigo_dane, // Asignar DANE central si no es adyacente
      ...
      return {
        measurement: {
          ...
          // Mantener compatibilidad pero ajustar significado o deprecarlos
          emisiones_autorizadas: autorizadas, // Todas las que tienen licencia
        ...
        stats: {
          ...
          cumplen: cumplen,
          fuera_parametros: fueraParametros,
          sin_licencia: sinLicencia,
          authorized: autorizadas, // Legacy
          unauthorized: sinLicencia // Legacy
        }
      };
    };

    // Procesar todas las mediciones en paralelo con límite de concurrencia
    // Flask corre con threaded=True, puede manejar ~25 requests simultáneas sin saturarse
    const CONCURRENCY_LIMIT = 25;
    console.log(`🚀 Starting parallel processing (concurrency=${CONCURRENCY_LIMIT}) with Python Flask backend...`);
    console.log(`📊 Total measurements to process: ${spectrumData.rows.length}`);

    // Semáforo simple: limita cuántas llamadas HTTP están activas a la vez
    let active = 0;
    let nextIndex = 0;
    ...
    const limitedProcessMeasurement = async (row: any, i: number) => {
      await acquire();
      try {
        return await processMeasurement(row, i);
      } finally {
        release();
        nextIndex++;
        if (nextIndex % 10 === 0 || nextIndex === spectrumData.rows.length) {
          console.log(`⏳ Progress: ${nextIndex}/${spectrumData.rows.length} measurements processed...`);
        }
      }
    };

    const allResults = await Promise.all(
      spectrumData.rows.map((row, i) => limitedProcessMeasurement(row, i))
    );

    for (const res of allResults) {
      if (res) {
        allMediciones.push(res.measurement);
        totalEmisionesGlobal += res.stats.total;
        totalAutorizadasGlobal += res.stats.authorized;
        totalSinLicenciaGlobal += res.stats.unauthorized;
        totalCumplenGlobal += (res.stats.cumplen || 0);
        totalFueraParametrosGlobal += (res.stats.fuera_parametros || 0);
      }
    }
    ...
    const porcentajeCumplimientoGlobal = totalEmisionesGlobal > 0 
      ...
      : '0';
    const medicionConDatosTecnicos = allMediciones.find((m: any) => m?.datos_tecnicos);
    ...
    const report = {
      ...
      datos_tecnicos: allMediciones.length > 0 ? {
        ...
        puntos_fft: medicionConDatosTecnicos?.datos_tecnicos?.puntos_fft || null,
        frecuencia_inicio_hz: medicionConDatosTecnicos?.datos_tecnicos?.frecuencia_inicio_hz || null,
        frecuencia_fin_hz: medicionConDatosTecnicos?.datos_tecnicos?.frecuencia_fin_hz || null,
        resolucion_hz: medicionConDatosTecnicos?.datos_tecnicos?.resolucion_hz || null
      } : null
    };
