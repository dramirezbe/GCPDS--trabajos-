...
- `POST /api/reports/compliance/batch/{campaignId}` - Listar sensores disponibles para ejecución por lote

### Resolución de DANE en el backend

El backend resuelve DANE dentro de `POST /api/reports/compliance/{campaignId}`. No existe un endpoint público dedicado para convertir coordenadas a DANE directamente.

Flujo implementado:

1. Obtiene coordenadas (campaña/sensor).
2. Llama al servicio interno ANE: `POST http://172.23.80.220:4155/localizar`.
3. Extrae `resultado.central.codigo_dane`.
4. Incluye también DANE(s) adyacente(s) en una lista `danes`.
5. Envía `dane` y `danes` al microservicio Python (`POST /analyze`).

Payload enviado al servicio de geolocalización:
...
Campos usados de la respuesta de geolocalización:
...
Opciones del endpoint `POST /api/reports/compliance/{campaignId}`:

- Path param:
  - `campaignId` (requerido)
- Query params:
  - `sensor_mac` (opcional)
  - `force=true` (opcional, fuerza regeneración)
  - `umbral` (opcional)
  - `umbral_db` (opcional)
- Body:
  - `umbral` (opcional)
...
Reglas de caché del reporte:

- Se usa caché solo si `force != true`, `umbral = 5` y no se envía `sensor_mac`.

Errores frecuentes relacionados con DANE/geolocalización:
