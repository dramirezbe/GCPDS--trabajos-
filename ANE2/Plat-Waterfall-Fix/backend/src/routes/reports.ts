...
router.post('/compliance/:campaignId', async (req: Request, res: Response) => {
  try {
    ...
    const processMeasurement = async (frameData: any, i: number) => {
      ...
      // Procesar mediciones en paralelo (lotes de 10)
    // El servidor Python usa Flask
    const BATCH_SIZE = 10;
    console.log(`🚀 Starting parallel processing (Batch Size ${BATCH_SIZE}) with Python Flask backend...`);
    
    for (let i = 0; i < spectrumData.rows.length; i += BATCH_SIZE) {
      const batch = spectrumData.rows.slice(i, i + BATCH_SIZE);
      console.log(`Processing batch ${Math.floor(i / BATCH_SIZE) + 1}/${Math.ceil(spectrumData.rows.length / BATCH_SIZE)}...`);
      
      const batchPromises = batch.map((row, idx) => processMeasurement(row, i + idx));
